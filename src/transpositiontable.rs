...
impl Index<usize> for TTCluster {
    type Output = TTEntry;
...
impl IndexMut<usize> for TTCluster {
...
impl TTClusterMemory {
    ...
    pub fn store(&self, cluster: TTCluster) {
        let ptr = self.inner.get();
        // Safety: Currently UB.
        unsafe { std::ptr::write(ptr, cluster) }
    }
    pub fn clear(&self) {
        let ptr = self.inner.get();
        // Safety: Currently UB.
...
impl TTView<'_> {
    ...
    pub fn store(
        ...
        // load the cluster:
        let cluster_ptr = &self.table[cluster_index];
        let mut cluster = cluster_ptr.load();
        let mut tte = cluster[0];
        let mut idx = 0;
    ...
    pub fn probe(&self, key: u64, ply: usize) -> Option<TTHit> {
        ...
        let cluster_ptr = &self.table[index];
        let cluster = cluster_ptr.load();
