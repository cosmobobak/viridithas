...
fn batch_convert(depth: i32, fens: &[String], evals: &mut Vec<i32>) {
    ...
    for fen in fens {
        ...
        let pov_score = pos.alpha_beta::<true, true>(&mut info, &mut t, Depth::new(depth), -INFINITY, INFINITY);
        let score = if pos.turn() == WHITE { pov_score } else { -pov_score };
        evals.push(score);
    }
}

pub fn wdl_to_nnue<P1: AsRef<Path>, P2: AsRef<Path>>(input_file: P1, output_file: P2) -> std::io::Result<()> {
    ...
    let evals = fens.par_chunks(chunk_size)
        .map(|chunk| {
